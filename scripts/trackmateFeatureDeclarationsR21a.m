...
% Jean-Yves Tinevez & contributors - 2026
    ...
    global isNotFirst docNode docNodeFileName %#ok<GVMIS>
    if isNotFirst
    ...
    if willClear || isempty(docNode) || ~strcmp(docNodeFileName, filePath)
        try
            docNode = matlab.io.xml.dom.Parser().parseFile( filePath );
        catch ME
    ...
    function ft = makeFeatureTable(featName, modelNode)
        prevW = warning('off', 'MATLAB:table:PreallocateCharWarning');
        ft = table( 'Size', [0 5], ...
        ...
        if strcmp('FeatureDeclarations', declNode.TagName)
            ...
            if strcmp(featName, fDNode.TagName)
                ...
                key = cell(neFeatures, 1);
                name = cell(neFeatures, 1);
                shortName = cell(neFeatures, 1);
                dimension = cell(neFeatures, 1);
