...
public class DetectorTestDrive
{
	public static <T extends RealType< T > & NativeType< T >>void main( final String[] args )
	{
		...
		final HyperSphere< T > sphere = new HyperSphere< T >( img, new Point( 32, 32 ), 5 );
		...
		for ( int i = 2; i < 25; i++ )
		{
			final double radius = i / 2d;
			final LogDetector< T > detector = new LogDetector< T >( img, img, calibration, radius, 1d, false, false );
			...
			try
			{
				if ( null != bestSpot )
				{
					...
					System.out.println( bestSpot.getFeature( Spot.RADIUS ) + "\t" + String.format( "%.1f", bestSpot.getFeature( Spot.QUALITY ) ) + "\t" + detector.getProcessingTime() ); // +
				}
			}
			finally
			{
