			...
			threads[i] = new Thread("TrackBranchingAnalyzer thread " + i) {
				...
				public void run() {
					...
					while ((trackID = queue.poll()) != null) {
						...
						final Set<Spot> track = model.getTrackModel()
								.trackSpots(trackID);
						...
						for (final Spot spot : track) {
							final Set<DefaultWeightedEdge> edges = model
									.getTrackModel().edgesOf(spot);

							// get neighbors
							final Set<TrackableObject> neighbors = new HashSet<TrackableObject>();
							for (final DefaultWeightedEdge edge : edges) {
								neighbors.add(model.getTrackModel()
										.getEdgeSource(edge));
								neighbors.add(model.getTrackModel()
										.getEdgeTarget(edge));
							}
							...
							for (final TrackableObject neighbor : neighbors) {
								if (TrackableObjectUtils.frameDiff(spot,
										neighbor) > 0) {
						...
						// Put feature data
						model.getFeatureModel().putTrackFeature(trackID,
								NUMBER_GAPS, Double.valueOf(ngaps));
						model.getFeatureModel().putTrackFeature(trackID,
								NUMBER_SPLITS, Double.valueOf(nsplits));
						model.getFeatureModel().putTrackFeature(trackID,
								NUMBER_MERGES, Double.valueOf(nmerges));
						model.getFeatureModel().putTrackFeature(trackID,
								NUMBER_COMPLEX, Double.valueOf(ncomplex));
						model.getFeatureModel().putTrackFeature(trackID,
								NUMBER_SPOTS, Double.valueOf(track.size()));
