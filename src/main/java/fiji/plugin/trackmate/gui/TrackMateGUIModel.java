...
public class TrackMateGUIModel
{
	...
	/**
	 * Is used to determine how to react to a 'next' button push. If it is set
	 * to true, then we are normally processing through the GUI, and pressing
	 * 'next' should update the GUI and process the data. If is is set to false,
	 * then we are currently loading/saving the data, and we should simply
	 ...
	 * <code>true</code>, then our state is "currently displaying display
	 * config" .
	 */
	...
	/**
	 * The panel descriptor previously displayed, e.g. before the user pressed
	 * the save button.
	 */
	WizardPanelDescriptor previousDescriptor;

	/**
	 * The collection of views instantiated and registered in the GUI. We need
	 * to keep track of them so that we can forward them the changes in display
	 * settings.
	 */
	 ...
	 *
	 * @param view
	 *            the {@link TrackMateModelView} to add.
	 */
	...
	/**
	 * Removes the specified {@link TrackMateModelView} to the collection of
	 * views managed by this GUI.
	 *
	 * @param view
	 *            the {@link TrackMateModelView} to remove.
	 */
	...
	/**
	 * Returns the key string of the current descriptor, representing the
	 * "state" of the GUI. Load, Save and Log descriptors are not returned; the
	 * descriptor that is displayed prior to call any of these 3 descriptor
	 * stands as the GUI state.
	 *
