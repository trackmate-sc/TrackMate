...
public class FilterGuiPanel extends JPanel implements ChangeListener {
    ...
    public void checkTargetType(final TrackMateObject target, final Model model, final Settings settings) {
//		switch()
    }

    public Map<String, String> trackMateObjectChecker(final TrackMateObject target, final Model model, final Settings settings) {
        Map<String, String> featureNames;
        FeatureUtils featureUtils;
        switch (target) {
            case SPOTS:
                featureUtils = new Spots();
                featureNames = featureUtils.collectFeatureKeys(target, model, settings);
                break;
            case EDGES:
    ...
    public void addFilterPanel(final FeatureFilter filter) {
        final Map<String, String> featureNames = trackMateObjectChecker(target, model, settings);
    ...
    private String guessNextFeature() {
        ...
        if (featureFilters.isEmpty())
            return (defaultFeature == null || !featureNames.containsKey(defaultFeature)) ? it.next() : defaultFeature;
