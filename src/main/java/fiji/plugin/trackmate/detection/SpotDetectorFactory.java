...
public interface SpotDetectorFactory< T extends RealType< T > & NativeType< T > > extends SpotDetectorFactoryBase< T >
{
	 ...
	 *            operate on. For {@link SpotDetector}, this must <b>not</b>
	 *            have a dimension for time (<i>e.g.</i> if the source image is
	 *            2D+T (3D), then the interval must be 2D; if the source image
