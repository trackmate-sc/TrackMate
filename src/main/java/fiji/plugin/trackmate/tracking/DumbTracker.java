...
public class DumbTracker implements Tracker<Spot> {
	...
	/**
	 * The number of links to create before we apply limitations based on
	 * previously created link distance statistics.
	 */
	...
	private final TrackableObjectCollection<Spot> spots;
	...
	public DumbTracker(final TrackableObjectCollection<Spot> spots) {
		this.spots = spots;
	}
	...
	public boolean process() {
		...
		final HashMap<Integer, List<Spot>> openStarts = new HashMap<Integer, List<Spot>>(
				frames.length);
		final HashMap<Integer, List<Spot>> openEnds = new HashMap<Integer, List<Spot>>(
				frames.length);
		...
		for (int i = 0; i < frames.length - 1; i++) {
			...
			final List<Spot> now = new ArrayList<Spot>(spots.getNObjects(
					frameSource, true));
			for (final Spot spot : spots.iterable(frameSource, true)) {
				now.add(spot);
			}
			final List<Spot> after = new ArrayList<Spot>(spots.getNObjects(
					frameTarget, true));
			...
			/*
			 * Find possible links between two sets. The method return the set
			 * of spot indices that have not been linked.
			 */
			...
			final List<Spot> sourceLeftOvers = new ArrayList<Spot>(
					indicesJ.size());
			...
			final List<Spot> targetLeftOvers = new ArrayList<Spot>(
					indicesK.size());
		...
		for (int delta = 2; delta < frames.length; delta++) {
			...
			for (int i = 0; i < frames.length - delta; i++) {
				...
				final List<Set<Integer>> unmatchedIndices = link(sources,
						targets);
				final Set<Integer> indicesSources = unmatchedIndices.get(0);
				final Set<Integer> indicesTargets = unmatchedIndices.get(1);

				/*
				 * Change the open ends and starts to only contain what has been
				 * linked
				 */

				final ArrayList<Spot> newSources = new ArrayList<Spot>(
						indicesSources.size());
				...
				final ArrayList<Spot> newTargets = new ArrayList<Spot>(
						indicesTargets.size());
	...
	private List<Set<Integer>> link(final List<Spot> now, final List<Spot> after) {
		...
		for (int j = 0; j < now.size(); j++) {
			...
			for (int k = 0; k < after.size(); k++) {
				...
				costs[j][k] = TrackableObjectUtils.squareDistanceTo(sa, sb);
			}
		}

		/*
		 * Loop over possible links until source or target spots are exhausted.
		 */
		...
		final List<Set<Integer>> unmatchedIndices = new ArrayList<Set<Integer>>(
				2);
