...
public class TrackSegmentCostMatrixCreator<T extends TrackableObject> extends LAPTrackerCostMatrixCreator {
	...
	private CostCalculator<T> costCalculator;
	...
	public TrackSegmentCostMatrixCreator(final CostCalculator<T> costCalculator, final List<SortedSet<T>> trackSegments, final Map<String, Object> settings) {
		super(settings);
	...
	public boolean process() {
		...
		try {
			...
			if (allowMerging || allowSplitting) {
				...
				middlePoints = new ArrayList<T>(0);
			...
			// Top left quadrant
			final Matrix topLeft = createTopLeftQuadrant();
			if (null == topLeft) {
				return false;
			}
			...
			final double cutoff = getCutoff(topLeft);
			final Matrix topRight = getAlternativeScores(topLeft.getRowDimension(), cutoff);
			final Matrix bottomLeft = getAlternativeScores(topLeft.getColumnDimension(), cutoff);
	 ...
	 */
	public List<T> getTrackSegmentMiddlePoints(final List<SortedSet<T>> trackSegments) {
		int n_spots = 0;
		for (final SortedSet<T> trackSegment : trackSegments) {
			n_spots += trackSegment.size();
		}
		final List<T> middlePoints = new ArrayList<T>(n_spots);
		for (final SortedSet<T> trackSegment : trackSegments) {
	...
	private Matrix createTopLeftQuadrant() {
		...
		final Matrix gapClosingScores = getGapClosingCostSubMatrix();
	...
	private Matrix getGapClosingCostSubMatrix() {
		final GapClosingCostFunction<T> gapClosing = new GapClosingCostFunction<T>(costCalculator, settings, trackSegments);
	...
	private Matrix getMergingCostSubMatrix() {
		final MergingCostFunction<T> merging = new MergingCostFunction<T>(costCalculator, settings, trackSegments, middlePoints);
		...
		if (PRUNING_OPTIMIZATION) {
			mergingMiddlePoints = new ArrayList<T>();
	...
	private Matrix getSplittingCostSubMatrix() {
		final SplittingCostFunction<T> splitting = new SplittingCostFunction<T>(costCalculator, settings, trackSegments, middlePoints); 
		...
		if (PRUNING_OPTIMIZATION) {
			splittingMiddlePoints = new ArrayList<T>();
	 ...
	 */
	private Matrix pruneColumns (final Matrix m, final List<T> keptMiddleSpots) {
		final double blockingValue = (Double) settings.get(KEY_BLOCKING_VALUE);
		...
		final double[][] pruned = new double[m.getRowDimension()][usefulColumns.size()];
		double[] col;
	 ...
	 */
	private Matrix pruneRows (final Matrix m, final List<T> keptMiddleSpots) {
		final double blockingValue = (Double) settings.get(KEY_BLOCKING_VALUE);
		...
		// Convert ArrayList<double[]> -> double[][] -> Matrix
		final double[][] pruned = new double[usefulRows.size()][m.getColumnDimension()];
	 ...
	 */
	private double getCutoff(final Matrix m) {
		final double blockingValue = (Double) settings.get(KEY_BLOCKING_VALUE);
