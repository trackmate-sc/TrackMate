...
public class LAPTrackerFactory implements TrackerFactory {
	...
	public static final String INFO_TEXT = "<html>"
			...
			+ "Jaqaman <i> et al.</i>, 2008, Nature Methods. <br>" + "</html>";
	...
	public ConfigurationPanel getTrackerConfigurationPanel(final Model model) {
		...
		final Collection<String> features = model.getFeatureModel()
				.getSpotFeatures();
		final Map<String, String> featureNames = model.getFeatureModel()
				.getSpotFeatureNames();
		return new LAPTrackerSettingsPanel(NAME, spaceUnits, features,
				featureNames);
	}
	...
	public boolean marshall(final Map<String, Object> settings,
		...
		ok = ok
				& writeAttribute(settings, linkingElement,
						KEY_LINKING_MAX_DISTANCE, Double.class, str);
		// feature penalties
		@SuppressWarnings("unchecked")
		final Map<String, Double> lfpm = (Map<String, Double>) settings
				.get(KEY_LINKING_FEATURE_PENALTIES);
		final Element lfpElement = new Element(
		...
		// Gap closing
		final Element gapClosingElement = new Element(
				XML_ELEMENT_NAME_GAP_CLOSING);
		ok = ok
				& writeAttribute(settings, gapClosingElement,
						KEY_ALLOW_GAP_CLOSING, Boolean.class, str);
		ok = ok
				& writeAttribute(settings, gapClosingElement,
						KEY_GAP_CLOSING_MAX_DISTANCE, Double.class, str);
		ok = ok
				& writeAttribute(settings, gapClosingElement,
						KEY_GAP_CLOSING_MAX_FRAME_GAP, Integer.class, str);
		// feature penalties
		@SuppressWarnings("unchecked")
		final Map<String, Double> gcfpm = (Map<String, Double>) settings
				.get(KEY_GAP_CLOSING_FEATURE_PENALTIES);
		final Element gcfpElement = new Element(
				XML_ELEMENT_NAME_FEATURE_PENALTIES);
		...
		// Track splitting
		final Element trackSplittingElement = new Element(
				XML_ELEMENT_NAME_SPLITTING);
		ok = ok
				& writeAttribute(settings, trackSplittingElement,
						KEY_ALLOW_TRACK_SPLITTING, Boolean.class, str);
		ok = ok
				& writeAttribute(settings, trackSplittingElement,
						KEY_SPLITTING_MAX_DISTANCE, Double.class, str);
		// feature penalties
		@SuppressWarnings("unchecked")
		final Map<String, Double> tsfpm = (Map<String, Double>) settings
				.get(KEY_SPLITTING_FEATURE_PENALTIES);
		final Element tsfpElement = new Element(
				XML_ELEMENT_NAME_FEATURE_PENALTIES);
		...
		// Track merging
		final Element trackMergingElement = new Element(
				XML_ELEMENT_NAME_MERGING);
		ok = ok
				& writeAttribute(settings, trackMergingElement,
						KEY_ALLOW_TRACK_MERGING, Boolean.class, str);
		ok = ok
				& writeAttribute(settings, trackMergingElement,
						KEY_MERGING_MAX_DISTANCE, Double.class, str);
		// feature penalties
		@SuppressWarnings("unchecked")
		final Map<String, Double> tmfpm = (Map<String, Double>) settings
				.get(KEY_MERGING_FEATURE_PENALTIES);
		final Element tmfpElement = new Element(
				XML_ELEMENT_NAME_FEATURE_PENALTIES);
		...
		// Others
		ok = ok
				& writeAttribute(settings, element, KEY_CUTOFF_PERCENTILE,
						Double.class, str);
		ok = ok
				& writeAttribute(settings, element,
						KEY_ALTERNATIVE_LINKING_COST_FACTOR, Double.class, str);
		ok = ok
				& writeAttribute(settings, element, KEY_BLOCKING_VALUE,
						Double.class, str);
	...
	public boolean unmarshall(final Element element,
		...
		// Linking
		final Element linkingElement = element
				.getChild(XML_ELEMENT_NAME_LINKING);
		if (null == linkingElement) {
			errorHolder.append("Could not found the "
			...
			ok = ok
					& readDoubleAttribute(linkingElement, settings,
							KEY_LINKING_MAX_DISTANCE, errorHolder);
			// feature penalties
			final Map<String, Double> lfpMap = new HashMap<String, Double>();
			final Element lfpElement = linkingElement
					.getChild(XML_ELEMENT_NAME_FEATURE_PENALTIES);
			if (null != lfpElement) {
		...
		// Gap closing
		final Element gapClosingElement = element
				.getChild(XML_ELEMENT_NAME_GAP_CLOSING);
		if (null == gapClosingElement) {
			errorHolder.append("Could not found the "
					+ XML_ELEMENT_NAME_GAP_CLOSING + " element in XML.\n");
			ok = false;

		} else {

			ok = ok
					& readBooleanAttribute(gapClosingElement, settings,
							KEY_ALLOW_GAP_CLOSING, errorHolder);
			ok = ok
					& readIntegerAttribute(gapClosingElement, settings,
							KEY_GAP_CLOSING_MAX_FRAME_GAP, errorHolder);
			ok = ok
					& readDoubleAttribute(gapClosingElement, settings,
							KEY_GAP_CLOSING_MAX_DISTANCE, errorHolder);
			// feature penalties
			final Map<String, Double> gcfpm = new HashMap<String, Double>();
			final Element gcfpElement = gapClosingElement
					.getChild(XML_ELEMENT_NAME_FEATURE_PENALTIES);
			if (null != gcfpElement) {
		...
		// Track splitting
		final Element trackSplittingElement = element
				.getChild(XML_ELEMENT_NAME_SPLITTING);
		if (null == trackSplittingElement) {
			errorHolder.append("Could not found the "
					+ XML_ELEMENT_NAME_SPLITTING + " element in XML.\n");
			ok = false;

		} else {

			ok = ok
					& readBooleanAttribute(trackSplittingElement, settings,
							KEY_ALLOW_TRACK_SPLITTING, errorHolder);
			ok = ok
					& readDoubleAttribute(trackSplittingElement, settings,
							KEY_SPLITTING_MAX_DISTANCE, errorHolder);
			// feature penalties
			final Map<String, Double> tsfpm = new HashMap<String, Double>();
			final Element tsfpElement = trackSplittingElement
					.getChild(XML_ELEMENT_NAME_FEATURE_PENALTIES);
			if (null != tsfpElement) {
		...
		// Track merging
		final Element trackMergingElement = element
				.getChild(XML_ELEMENT_NAME_MERGING);
		if (null == trackMergingElement) {
			errorHolder.append("Could not found the "
					+ XML_ELEMENT_NAME_MERGING + " element in XML.\n");
			ok = false;

		} else {

			ok = ok
					& readBooleanAttribute(trackMergingElement, settings,
							KEY_ALLOW_TRACK_MERGING, errorHolder);
			ok = ok
					& readDoubleAttribute(trackMergingElement, settings,
							KEY_MERGING_MAX_DISTANCE, errorHolder);
			// feature penalties
			final Map<String, Double> tmfpm = new HashMap<String, Double>();
			final Element tmfpElement = trackMergingElement
					.getChild(XML_ELEMENT_NAME_FEATURE_PENALTIES);
			if (null != tmfpElement) {
		...
		// Others
		ok = ok
				& readDoubleAttribute(element, settings, KEY_CUTOFF_PERCENTILE,
						errorHolder);
		ok = ok
				& readDoubleAttribute(element, settings,
						KEY_ALTERNATIVE_LINKING_COST_FACTOR, errorHolder);
		ok = ok
				& readDoubleAttribute(element, settings, KEY_BLOCKING_VALUE,
						errorHolder);
	...
	public String toString(final Map<String, Object> sm) {
		...
		str.append(LAPUtils.echoFeaturePenalties((Map<String, Double>) sm
				.get(KEY_LINKING_FEATURE_PENALTIES)));
