...
public class TrackSchemeSvgCanvas extends mxSvgCanvas
{
	...
	public Element drawShape( int x, int y, int w, int h, Map< String, Object > style )
	{
		...
		{
			String r = String.valueOf( Math.min( w * mxConstants.RECTANGLE_ROUNDING_FACTOR, h * mxConstants.RECTANGLE_ROUNDING_FACTOR ) );
		...
		if ( mxUtils.isTrue( style, mxConstants.STYLE_GLASS, false ) )
		{
			...
			// LATER: Not sure what the behaviour is for mutiple SVG elements in
			// page.
			// Probably its possible that this points to an element in another
			// SVG
