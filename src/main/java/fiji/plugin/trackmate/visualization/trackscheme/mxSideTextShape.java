 ...
 * Works only if the chosen edge style is
 * {@link com.mxgraph.shape.mxConnectorShape}.
 * 
...
public class mxSideTextShape extends mxDefaultTextShape
{
	...
	@Override
	public void paintShape( final mxGraphics2DCanvas canvas, final String text, final mxCellState state, final Map< String, Object > style )
	{

		final mxCell cell = ( mxCell ) state.getCell();
		final boolean isEdgeLabel = cell.isEdge();

		final Rectangle rect = state.getLabelBounds().getRectangle();
		final Graphics2D g = canvas.getGraphics();

		if ( g.getClipBounds() == null || g.getClipBounds().intersects( rect ) )
		{
			final boolean horizontal = mxUtils.isTrue( style,
			...
			if ( isEdgeLabel )
			{
				...
				final double px1 = points.get( 0 ).getX();
				final double py1 = points.get( 0 ).getY();
				final double px2 = points.get( 1 ).getX();
				final double py2 = points.get( 1 ).getY();
				final double dx = px2 - px1;
				final double dy = py2 - py1;
				final double theta = dx > 0 ? Math.atan2( dy, dx ) : Math.atan2( dy, dx ) + Math.PI;
			...
			final Color fontColor = mxUtils.getColor( style, mxConstants.STYLE_FONTCOLOR, Color.black );
			g.setColor( fontColor );

			// Shifts the y-coordinate down by the ascent plus a workaround
			// for the line not starting at the exact vertical location
			final Font scaledFont = mxUtils.getFont( style, scale );
			g.setFont( scaledFont );
			final int fontSize = mxUtils.getInt( style, mxConstants.STYLE_FONTSIZE, mxConstants.DEFAULT_FONTSIZE );
			final FontMetrics fm = g.getFontMetrics();
			final int scaledFontSize = scaledFont.getSize();
			final double fontScaleFactor = ( ( double ) scaledFontSize ) / ( ( double ) fontSize );
			...
			// when scaled, etc.
			final double fontScaleRatio = fontScaleFactor / scale;
			...
			for ( int i = 0; i < lines.length; i++ )
			{
				...
				{
					final int sw = fm.stringWidth( lines[ i ] );
